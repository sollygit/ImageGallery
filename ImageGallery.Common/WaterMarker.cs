...
namespace ImageGallery.Common
{
    ...
    {
        const string Watermark = "(c) ImageGallery";

        public static void Generate(Stream originalImageStrm, Stream newImageStrm)
        {
            ...
            using (Graphics graphic = Graphics.FromImage(inputImage))
            {
                ...
                graphic.DrawString(Watermark, font, semiTransBrush2, xPostFromLeft + 1, yPosFromBottom + 1, StrFormat);

                SolidBrush semiTransBrush = new SolidBrush(Color.FromArgb(153, 255, 255, 255));
                graphic.DrawString(Watermark, font, semiTransBrush, xPostFromLeft, yPosFromBottom, StrFormat);
                graphic.Flush();
